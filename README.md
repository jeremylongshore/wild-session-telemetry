...
Part of the **[wild ecosystem](https://github.com/intent-solutions-io/wild-rails-ai-ops)** — 10 Ruby gems for running AI agents inside Rails apps under capability control. Receives structured events from operational repos (`wild-admin-tools-mcp`, `wild-rails-safe-introspection-mcp`) and produces aggregated, privacy-safe usage data for downstream analysis by `wild-gap-miner` and `wild-transcript-pipeline`.
